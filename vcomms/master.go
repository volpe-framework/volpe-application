...
type masterCommsServer struct {
	vc.UnimplementedVolpeMasterServer
	chans_mut  sync.RWMutex
	channs     map[string]chan *vc.MasterMessage
	metricChan chan *vc.DeviceMetricsMessage
	immigChan    chan *vc.MigrationMessage
...
func mcsStreamHandlerThread(
	workerID string,
	stream grpc.BidiStreamingServer[vc.WorkerMessage, vc.MasterMessage],
	masterSendChan chan *vc.MasterMessage,
	metricChan chan *vc.DeviceMetricsMessage,
	immigChan chan *vc.MigrationMessage,
	eventStream chan string,
) {
	...
	masterRecvChan := make(chan *vc.WorkerMessage)
...
func initMasterCommsServer(mcs *masterCommsServer, metricChan chan *vc.DeviceMetricsMessage, eventStream chan string) (err error) {
	mcs.channs = make(map[string]chan *vc.MasterMessage)
	mcs.metricChan = metricChan
	mcs.eventStream = eventStream
	return nil
}

func (mcs *masterCommsServer) StartStreams(stream grpc.BidiStreamingServer[vc.WorkerMessage, vc.MasterMessage]) error {
	...
	masterSendChan := make(chan *vc.MasterMessage)
...
func (mcs *masterCommsServer) GetProblemData(req *vc.ProblemRequest, stream grpc.ServerStreamingServer[common.ImageStreamObject]) error {
...
func NewMasterComms(port uint16, metricChan chan *vc.DeviceMetricsMessage, immigChan chan *vc.MigrationMessage, sched SchedulerComms, probStore ProblemStore, eventStream chan string) (*MasterComms, error) {
	mc := new(MasterComms)
	...
	vc.RegisterVolpeMasterServer(sr, &mc.mcs)
...
func (mc *MasterComms) SendMasterMessage(workerID string, msg *vc.MasterMessage) error {
