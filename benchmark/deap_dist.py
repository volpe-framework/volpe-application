...
P_MUTATION = 0.2
MAXTIME = 200
ISLAND_POPULATION_SIZE = POPULATION_SIZE//8

func = cec2022.F122022(ndim=DIMENSIONS)
...
def main():
    ...
    for func_name in ["F12"]:
        ...
        for run_no in range(1, 11):
            ...
            masterPop = toolbox.population(n=ISLAND_POPULATION_SIZE)
            islands = [toolbox.population(n=ISLAND_POPULATION_SIZE) for _ in range(8)]
            ...
            for isl in islands:
                fitnesses = list(toolbox.map(toolbox.evaluate, isl))
                for ind, fit in zip(isl, fitnesses):
                    ind.fitness.values = fit

            fitnesses = list(toolbox.map(toolbox.evaluate, masterPop))
            for ind, fit in zip(masterPop, fitnesses):
                ind.fitness.values = fit
                ...
                with open(output_filename, 'a') as f:
                    ...
                    while True:
                        for i in range(len(islands)):
                            islands[i], _ = algorithms.eaSimple(
                                islands[i], toolbox,
                        ...
                        cur = time.time()
                        if cur - last_write > 5:
                            selected = [toolbox.select(isl, 10) for isl in islands]
                            for sel in selected:
                                masterPop = toolbox.select(masterPop + sel, len(masterPop))
                            selected = [toolbox.select(masterPop, 10) for _ in islands]
                            for i in range(len(islands)):
                                islands[i] = toolbox.select(islands[i] + selected[i], len(islands[i]))
                            best_ind = hof[0]
